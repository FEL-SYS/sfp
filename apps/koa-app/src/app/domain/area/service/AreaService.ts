...
export class AreaService {
	...
	async list(
		...
		@Path() lastId: number,
		@Path() sort: number,
		@Query() search?: string): Promise<PaginationResult<Area>> {
		const where = {
			area: {
				contains: search
			}
		}
		const total = await prisma.area.count({ where, orderBy: { area: 'asc' } });
	...
	async view(
		...
		if (!data) {
			throw new HttpException('Data tidak ditemukan', HTTP_STATUS.NOT_FOUND);
		}
		return data;
	}
	...
	async delete(
		...
		if (data) {
			throw new HttpException('Gagal menghapus data', HTTP_STATUS.CONFLICT);
		}
		return data;
	}
}
